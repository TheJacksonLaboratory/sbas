...
genMCMC = function( data , xName="x" , yName="y" , 
  ...
  dataList = list(
    x = x ,
    y = y ,
    Nx = dim(x)[2] ,  #Number of data types like height, weight etc.
    Ntotal = dim(x)[1] #Number of data *points*, which includes a point for each tuple of 
  ...
  #They standardize the weight, height etc. to have mean 0 and standard deviation 1
  ...
  model {
    ...
    }  #note that ilogit is inside dbern, because they are only interested in getting the 
       #posteriors for the miu-i parameters(normal), not the beta-i(bernoulli)
       #Also note that zx in the expression within ilogit is a constant value not a parameter
