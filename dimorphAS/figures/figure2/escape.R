...
colnames(res.exp)<-c('Tissue','Esacpe-Biased','Inactive-Biased','Variable-Biased')
...
colnames(res.as)<-c('Tissue','Esacpe-Biased','Inactive-Biased','Variable-Biased')
...
levels(df$Type)[levels(df$Type)=='Esacpe'] <- "Escape"
...
# pval.as is 2.463e-20
# pval.exp is  5.47e-50

as.label = expression(italic(p)==2.43  %*% 10 ^ -20)
exp.label = expression(italic(p)==5.47 %*% 10 ^ -50)
...
pn <- p + scale_fill_npg()  + theme_minimal() 
pn <- pn + theme(text = element_text(size=20),
...
#pn <- pn + scale_color_manual(labels = c("a", "b", "c"), values = c(npgBlue,npgRed, npgGreen)) 
pn<-pn+ annotate("text", x=1, y=0.57, label=as.label , parse=TRUE, size=6)
pn<-pn+ annotate("text", x=3, y=0.72, label=exp.label,parse=TRUE, size=6)
pn <- pn + scale_y_continuous(labels = scales::percent)
