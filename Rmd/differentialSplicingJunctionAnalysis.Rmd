...
makeSplicingExpressionSetObject <- function (gtexPhenoDataObj, srr_metadata, filename_gz) {
    ...
    message("\nloading ", paste(counts_filename_gz), collapse=" ")
    counts <- data.table::fread(counts_filename_gz)
    message("done!")
    ...
    counts_for_metadata = srr__metadata$Run %in% colnames(counts)
    ...
    srr_metadata <- srr_metadata[counts_for_metadata==TRUE,                         ]
    ...
    j = order(srr_metadata$Run)

    counts        = counts[,i]
    srr__metadata = srr_metadata[j,]

   
    # phenotype data will be repeated - for samples that are shared
    # because of that we have to build the matrix
    # build the matrix from the first one, rbind within the loop
    sample_match = srr_metadata[colnames(counts[,1]) == metadata$Run,]
    pd     <- pData(gtexPhenoDataObj[gtexPhenoDataObj$SAMPID==metadata$SAMPID,])
    pd$SRR <- metadata$Run
    pdfinal <- pd
    for (i in 2:dim(counts)[2]) {
        sample_match = srr_metadata[colnames(counts[,i]) == metadata$Run,]
        pd     <- pData(gtexPhenoDataObj[gtexPhenoDataObj$SAMPID==metadata$SAMPID,])
        pd$SRR <- metadata$Run
...
inspectAndCorrectExpressionSetObject <- function ( es ) {
   ...
   message("\nAFTER: phenotype of expressionSet Esect\n",
        paste(tail (pData(es),2)), collapse=" ")
   message("\nAFTER: dimension of expressionSet Esect\n",
...
# Main routine
#
# get the released rmats matrices (rows junction ids, columns accessions (SRR)
# 
getReleasedRMATSData (destDir <- "../data/")

#
# 1.2.1 get the rmats 3.2.5 discovered/annoated junction information in GTF format
#
getReleasedGTFAnnotations (destDir <- "../data")

#
# 1.2.2 get released GTF Annotations (destDir)
#
getReleasedGTFAnnotations (destDir <- "../data/")
...
#
# 1.2.4 inspect and correct GTEx ExpressionSet obj
#
...
#
# 1.3.3 Eliminate ChrY
#
obj <- eliminateChrY (obj, fromGTF) 
...
pData(obj)$SAMPID   <- gsub('-','\\.',pData(reduced_obj)$SAMPID)
