...
```
### 1.2.7 read in and make Splicing Expression Set Object
...
```{r}
    ...
    message("\n SraRunTable match to SRR counts\n",
            paste(table(srr_metadata_match)), collapse = " ")
    srr_metadata_sampid <- srr_metadata[srr_metadata_match==TRUE,]$SAMPID

    write.csv(pData(gtexPhenoDataObj), "../data/pData_gtex_corrected_obj.csv", quote=FALSE, row.names=FALSE)

    pData(gtexPhenoDataObj)$SAMPID <- gsub('-','\\.',pData(gtexPhenoDataObj)$SAMPID)
    pd_sampid_match <- as.character(pData(gtexPhenoDataObj)$SAMPID) %in% as.character(srr_metadata_sampid)
    message("\n counts SAMPID match to phenotype Data SAMPID\n",
    ...
    for (i in 2:dim(counts)[2]) {
        srr_metadata_match <- as.character(srr_metadata$Run) %in% as.character(count_srrs[i])
        message("\n SraRunTable match to SRR counts\n",
            paste(table(srr_metadata_match)), collapse = " ")
        srr_metadata_sampid <- srr_metadata[srr_metadata_match==TRUE,]$SAMPID
        pData(gtexPhenoDataObj)$SAMPID <- gsub('-','\\.',pData(gtexPhenoDataObj)$SAMPID)
...
#
obj <- getGTExCorrectedRDS (destDir <- "../data/")
