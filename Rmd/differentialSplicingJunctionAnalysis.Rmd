...
execute_models_A_and_B <- function (plot, dup, tissue_of_interest, splice_type, fromGTF, tissue_list, ijc, sjc) {
    ...
    message("count_threshold\n",
           count_threshold)
    message("table(pData(ijc)$SEX)\n",
           table(pData(ijc)$SEX))
    
    male.sjc.rs = rowSums(cpm(exprs(sjc[,pData(ijc)$SEX==1]))>=1)
    
    male.ijc.rs = rowSums(cpm(exprs(ijc[,pData(ijc)$SEX==1]))>=1)
    
    female.sjc.rs = rowSums(cpm(exprs(sjc[,pData(ijc)$SEX==2]))>=1)
    
    female.ijc.rs = rowSums(cpm(exprs(ijc[,pData(ijc)$SEX==2]))>=1)
    
    keep= (male.sjc.rs>=count_threshold)  & (male.ijc.rs>=count_threshold) & (female.sjc.rs>=count_threshold) & (female.ijc.rs>=count_threshold)
    ...
    sex_specific_junctions_filename         = paste0(paste0(paste0(csv_sub_directory, splice_type),
                                                   snakecase::to_snake_case(tissue_of_interest)),'_AS_model_B_sex_specific_junctions.csv')

    keep2= !keep & ((male.sjc.rs>=count_threshold & female.ijc.rs>=count_threshold) | (male.ijc.rs>=count_threshold & female.sjc.rs>=count_threshold))
    
    write.table(rownames(sjc)[keep2], file = sex_specific_junctions_filename, 
...
write.table(tissue_list, "../data/tissue_list.csv", sep=",", quote=FALSE, row.names=TRUE, col.names=FALSE)
   splice_type = "a3ss_"
    ...
    #
    minimum_sample_size = min(table(pData(ijc)$SEX))*0.25
    message("minimum_sample_size\n",
           minimum_sample_size)

    #
    # now using the expression object function, we get the number of samples
    # in each set that have expression counts (normalized as counts per million) greater than 1
    # for each gender set and for each count matrices (sjc and ijc).  Otherwise we will
    # ignore the associated junction.
    #
    male.ijc.num_of_samples_gt_1 = rowSums(cpm(exprs(ijc[,pData(ijc)$SEX==1]))>=1)
    male.sjc.num_of_samples_gt_1 = rowSums(cpm(exprs(sjc[,pData(sjc)$SEX==1]))>=1)
    
    female.ijc.num_of_samples_gt_1 = rowSums(cpm(exprs(ijc[,pData(ijc)$SEX==2]))>=1)
    female.sjc.num_of_samples_gt_1 = rowSums(cpm(exprs(sjc[,pData(sjc)$SEX==2]))>=1)

    #
    # we keep only junctions for each sex, that have either a minimum number of samples in
    # either the sjc or ijc counts)
    #
    keep.male.ijc   = (  male.ijc.num_of_samples_gt_1 >= count_threshold)
    keep.male.sjc   = (  male.sjc.num_of_samples_gt_1 >= count_threshold)
    keep.female.ijc = (female.ijc.num_of_samples_gt_1 >= count_threshold)
    keep.female.sjc = (female.sjc.num_of_samples_gt_1 >= count_threshold)

    keep.male   <- keep.male.ijc | keep.male.sjc
    keep.female <- keep.female.ijc | keep.female.sjc
    #
    # and we keep only those junctions that have at least a minimum in each gender
    #
    keep  <- keep.male & keep.female
